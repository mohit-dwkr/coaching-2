...
export default function GallerySection({ data: gallery = [] }) {


  // Note: Loading aur Fetching logic ab Index.tsx handle kar raha hai.
  const [limit, setLimit] = useState(6);

  const displayedImages = gallery.slice(0, limit);

  const handleLoadMore = () => {
    setLimit((prev) => prev + 6); // Har click par 6 naye photos
  };
  ...
  return (
    <section id="gallery" className="py-10 bg-white relative overflow-hidden">
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
          {gallery.length > 0 ? (
            displayedImages.map((img, i) => (
              ...
              >
                {/* Image with smooth zoom and lazy loading */}
                <img
                  src={img.image_url}
                  alt={img.caption}
                  loading="lazy"
        ...
        {/* Updated Load More / Show Less Logic */}
        {gallery.length > 6 && (
          <div className="mt-10 flex justify-center">
            {limit < gallery.length ? (
              ...
              >
                ...
                <div className="flex items-center gap-3">
                  Explore More Photos <ChevronDown className="h-5 w-5 transition-transform group-hover:translate-y-1" />
                </div>
              </Button>
            ) : (
              <Button
                ...
                className="group h-16 rounded-[2rem] px-12 text-gray-500 hover:text-primary font-black uppercase tracking-[0.15em] text-[10px] md:text-xs transition-all duration-300"
              >
                <div className="flex items-center gap-3">
